...
class DailyJournalGenerator {
    ...
    generateTodaysEntry(workItems = [], pullRequests = [], meetings = []) {
        ...
        const entry = {
            ...
            accomplishments: [
                ...
                ...workItemCategories.inProgress.map(item => `Continued work on ${item.workItemType}: ${item.title} (ID: ${item.id})`)
            ],
            challenges: [],
            nextSteps: [
                // Pre-populate with open work items
                ...workItemCategories.open.map(item => `Start/continue ${item.workItemType}: ${item.title} (ID: ${item.id})`)
            ],
...
${entry.workItems.inProgress.length === 0 ? '_No items in progress_' :
            entry.workItems.inProgress.map(item => `- **${item.workItemType} #${item.id}**: ${item.title}${item.tags ? `\\n  _Tags: ${item.tags}_` : ''}`).join('\\n')}

### 📂 Open Items
${entry.workItems.open.length === 0 ? '_No open items_' :
            entry.workItems.open.map(item => `- **${item.workItemType} #${item.id}**: ${item.title}${item.tags ? `\\n  _Tags: ${item.tags}_` : ''}`).join('\\n')}

### ⏸️ Pending Items
${entry.workItems.pending.length === 0 ? '_No pending items_' :
            entry.workItems.pending.map(item => `- **${item.workItemType} #${item.id}**: ${item.title}${item.tags ? `\\n  _Tags: ${item.tags}_` : ''}`).join('\\n')}
...
export { DailyJournalGenerator };
//# sourceMappingURL=journal-generator.js.map