...
import { CoreApi } from "azure-devops-node-api/CoreApi.js";
...
class AzureDevOpsService {
  ...
  async getWorkItemTrackingApi(): Promise<WorkItemTrackingApi> {
    return await this.connection.getWorkItemTrackingApi();
  }

  async getCoreApi(): Promise<CoreApi> {
  ...
  async getBuildApi(): Promise<BuildApi> {
        ...
        content: [
          {
            ...
            text: `Found ${builds.length} recent builds in project "${project}":\n\n` +
                  builds.map(build => 
                    `• Build #${build.buildNumber} (${build.definition?.name})\n` +
                    `  Status: ${build.status}\n` +
